...
void GameInitializer::initialize_new_round(GameState *game, Queue<Message> &message_queue){
    ...
    if(message_map.type == MAP_INICIALIZATION){
        std::cout << "Recibo el mapa de nuevo "<< "\n";
    ...
    if(message_spawn.type == SPAWN_PLACE_POSITION){

        game->spawn_places.clear();

        std::cout << "HORA DE RECIBIR SPAWNS de nuevo" << "\n";
