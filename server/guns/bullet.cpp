...
Bullet::Bullet(int start_x, int start_y, int direction_x, int direction_y, GameMap* map) 
    : position_x(start_x), position_y(start_y), direction_x(direction_x), direction_y(direction_y), map(map), impacto(false) {
...
void Bullet::update_position() {
    ...
    if (map->canMoveBulletTo(delta_x, delta_y)) { 
        position_x = delta_x;
        position_y = delta_y;
